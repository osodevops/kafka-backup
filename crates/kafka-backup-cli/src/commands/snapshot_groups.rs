//! snapshot-groups command.
//!
//! Lists all consumer groups from every broker in the source Kafka cluster,
//! fetches their committed offsets, filters to groups that have offsets on
//! backed-up topics, and saves the result to
//! `{backup_id}/consumer-groups-snapshot.json` in the configured storage
//! backend.
//!
//! This file replaces the Python `snapshot_groups.py` workaround.
...
/// Saved format for one consumer group's offsets.
///
/// Keeps the same JSON shape as the Python snapshot_groups.py output so that
/// existing consumers of the file continue to work without changes.
#[derive(Serialize)]
struct GroupOffsets {
    group_id: String,
...
pub async fn run(config_path: &str) -> Result<()> {
    ...
    // Connect to source cluster via partition-leader router (so we can call
    // all brokers individually for KRaft-safe group listing).
    info!("Connecting to source Kafka cluster to list consumer groups...");
    ...
    info!(
        "Manifest loaded: {} backed-up topics",
        backed_topics.len()
    );

    // List all consumer groups across ALL brokers (KRaft: each broker is
    // group coordinator only for a subset of groups)
    let all_groups = router.list_groups_all_brokers().await?;
    info!("Found {} consumer groups across all brokers", all_groups.len());

    // Use the bootstrap client for OffsetFetch requests
    // (broker routes to correct coordinator internally)
    let bootstrap_client = {
        // Re-connect a lightweight bootstrap client
        kafka_backup_core::kafka::KafkaClient::new(source.clone())
    };
    bootstrap_client.connect().await?;

    let mut snapshot_groups: Vec<GroupOffsets> = Vec::new();

    for group in &all_groups {
        let group_id = &group.group_id;

        // Fetch all committed offsets for this group (no topic filter)
        ...
        // Filter to offsets that are on backed-up topics
        ...
        snapshot_groups.push(GroupOffsets {
    ...
    info!(
        "Saving snapshot for {} consumer groups to storage",
        snapshot_groups.len()
    );
