...
use crate::offset_headers::{X_ORIGINAL_OFFSET, X_ORIGINAL_TIMESTAMP, X_SOURCE_CLUSTER};
            ...
            PartitionCoverageState::Missing => {
                ...
                if recovery {
                    report.errors.push(format!(
                        "{msg}. Consumer-offset recovery is only performed for archives that \
                         carry the Phase 1 tracking headers (backup.include_offset_headers, on \
                         by default) as proof of provenance; re-run the backup with them enabled, \
                         or set restore.header_preflight: skip to proceed on the segment offsets \
                         alone. See docs/configuration.md#offset-tracking-headers."
                    ));
                } else {
...
fn inspect_record(record: &BackupRecord, cov: &mut PartitionHeaderCoverage) {
    ...
    for header in &record.headers {
        // A null header value (`None`) carries no offset — it must not count
        // as coverage, mirroring the restore engine's `decode_i64_header`.
        let decodable = || header.value.as_deref().is_some_and(decodes_as_i64);
        match header.key.as_str() {
            X_ORIGINAL_OFFSET => offset = offset || decodable(),
            X_ORIGINAL_TIMESTAMP => timestamp = timestamp || decodable(),
            X_SOURCE_CLUSTER => cov.records_with_source_cluster_header += 1,
            _ => {}
        }
    }
    if offset {
...
mod tests {
    ...
    fn empty_coverage() -> PartitionHeaderCoverage {
        PartitionHeaderCoverage {
            topic: "t".to_string(),
            partition: 0,
            state: PartitionCoverageState::Indeterminate,
    ...
    /// A tracking header whose value is null (issue #155) proves nothing:
    /// the record must not count as covered.
    #[test]
    fn null_tracking_header_values_do_not_count_as_coverage() {
        use crate::manifest::{BackupRecord, RecordHeader};
        let header = |key: &str, value: Option<Vec<u8>>| RecordHeader {
            key: key.to_string(),
            value,
        };
        ...
        inspect_record(
            &BackupRecord {
                ...
                value: None,
                headers: vec![
                    header(X_ORIGINAL_OFFSET, None),
                    header(X_ORIGINAL_TIMESTAMP, None),
                    header(X_SOURCE_CLUSTER, Some(b"eu".to_vec())),
                ],
                timestamp: 0,
                offset: 5,
            },
        ...
        assert_eq!(cov.records_with_offset_header, 0);
        assert_eq!(cov.records_with_timestamp_header, 0);
        assert_eq!(cov.records_with_required_headers, 0);
        assert_eq!(cov.records_with_source_cluster_header, 1);
        ...
        inspect_record(
            &BackupRecord {
                ...
                value: None,
                headers: vec![
                    header(X_ORIGINAL_OFFSET, Some(5i64.to_le_bytes().to_vec())),
                    header(X_ORIGINAL_TIMESTAMP, Some(b"1700000000000".to_vec())),
                ],
                timestamp: 0,
                offset: 5,
            },
        ...
        assert_eq!(cov.records_with_required_headers, 1);
    }
}
