...
/// Write one binary segment whose records carry **null-valued** tracking
/// headers (what a pre-0.18.0 archive could not express, and what a broker may
/// legitimately hand back), and return its manifest metadata.
async fn write_binary_segment_with_null_headers(
    dir: &Path,
    partition: i32,
    start_offset: i64,
    count: i64,
    ...
    let mut writer = SegmentWriter::new(SegmentWriterConfig::default(), storage, metrics);
    for i in 0..count {
        let mut r = record(start_offset + i, false);
        r.headers
            .push((offset_headers::X_ORIGINAL_OFFSET.to_string(), None));
        r.headers
            .push((offset_headers::X_ORIGINAL_TIMESTAMP.to_string(), None));
        ...
        .map(|i| {
            ...
            if with_headers {
                ...
                headers.push(RecordHeader {
                    ...
                    value: Some((1_700_000_000_000 + offset).to_le_bytes().to_vec()),
                });
            }
            BackupRecord {
...
/// Issue #155 follow-up: a tracking header that is present but **null** is not
/// coverage. Such a partition must classify as `Missing`, never `Full`.
#[tokio::test]
async fn null_valued_tracking_headers_do_not_count_as_coverage() {
    let dir = TempDir::new().unwrap();
    let seg = write_binary_segment_with_null_headers(dir.path(), 0, 0, 12).await;
    let manifest = write_manifest(
        dir.path(),
        vec![PartitionBackup {
            partition_id: 0,
    ...
    assert!(
        !report.passed,
        "null header values must not pass: {report:?}"
    );
    assert_eq!(report.partitions[0].state, PartitionCoverageState::Missing);
    assert_eq!(report.partitions[0].records_scanned, 12);
    assert_eq!(report.partitions[0].records_with_required_headers, 0);
}
