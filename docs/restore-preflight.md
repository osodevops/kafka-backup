...
- per-record `x-original-offset` / `x-original-timestamp` headers (and the
  optional `x-source-cluster` header) written by the backup when
  `backup.include_offset_headers` is on — which it is by default; see
  [Offset-tracking headers](configuration.md#offset-tracking-headers) for what
  each side adds. A header whose value is null (archives written before v0.18.0
  could not represent null) does not count as coverage;
...
`restore.strip_offset_headers` never affects the preflight: stripping is a
produce-time transform applied after the archive has been read, and the offset
mapping used by Phase 3 comes from the offset stored natively in every segment
record. The preflight requires the headers as proof of provenance, not as the
mapping's data source.
